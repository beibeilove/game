...
//游戏的主类
function game(canvas,cobj,runs,jumps,hinderImg,runA,bgA,dieA,jumpA,suiA,hitA){
    this.canvas=canvas;
    this.cobj=cobj;
    this.hinderImg=hinderImg;
            ...
            for(var i=0;i<that.hinderArr.length;i++){
                ...
                if(hitPix(that.canvas,that.cobj,that.person,that.hinderArr[i])){
                    ...
                    if(that.hinderArr[i].flag4){
                        xue(that.cobj,that.person.x+that.person.width/2,that.person.y+that.person.height/2,"red");
                        that.score+=2;
                        that.hinderArr[i].flag4=false;
                    }
                    if(that.hinderArr[i].flag5){
                        xue(that.cobj,that.person.x+that.person.width/2,that.person.y+that.person.height/2,"red");
                        that.hinderArr[i].flag4=false;
                    }
                }
                ...
                if(that.isfire){
                    ...
                    if(hitPix(that.canvas,that.cobj,that.zidanArr[that.zidanNum],that.hinderArr[i])){
                        ...
                        if(!that.hinderArr[i].flag){
                            xue(that.cobj,that.hinderArr[i].x+that.hinderArr[i].width/2,that.person.y+that.person.height/2,"#ccc");
                            document.querySelectorAll("span")[2].innerHTML = that.person.life;
                            that.hinderArr[i].flag1=true;
                        }
                        if(that.hinderArr[i].flag2){
                            xue(that.cobj,that.hinderArr[i].x+that.hinderArr[i].width/2,that.person.y+that.person.height/2,"#000");
                            document.querySelectorAll("span")[2].innerHTML=that.person.life;
                            that.hinderArr[i].flag2=false;
                            that.hinderArr[i].flag1=true;
                        }
                        ...
                        if(that.hinderArr[i].flag4){
                            xue(that.cobj,that.person.x+that.person.width/2,that.person.y+that.person.height/2,"orange");
                            that.hinderArr[i].flag4=false;
                        }
                        if(that.hinderArr[i].flag5){
                            that.score--;
